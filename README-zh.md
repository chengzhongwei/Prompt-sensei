...
然后用自然语言告诉 Codex：`Use prompt-sensei observe mode.` 如果想开启可选 auto-start，可以运行 `npm run setup-hooks -- auto-observe user`，或在 skill 加载后使用 `/prompt-sensei setup`。
...
Setup 会处理 observe 授权、可选的宿主 auto-start hooks，以及是否保存脱敏 prompt preview。在 Codex 里，auto-start 使用 Codex lifecycle hooks，可能需要用 `/hooks` 审核并信任新 hooks。Redacted previews 默认保持关闭，除非你明确打开。高级细节见 [docs/advanced-setup-zh.md](docs/advanced-setup-zh.md)。
...
Auto observe 和 redacted prompt previews 都必须主动开启。Auto observe 在 Claude Code 里使用 Claude Code hooks，在 Codex 里使用 Codex lifecycle hooks。用 `/prompt-sensei setup` 可以走引导式设置；完整设置参考见 [docs/advanced-setup-zh.md](docs/advanced-setup-zh.md)。

设置命令也支持更灵活的写法，比如 `auto-observe=off save-redacted-prompts=on`、`save redacted prompts enable`，以及 `redacted`、`previews`、`auto-start` 这类别名。
...
宿主 hooks 可以提供 opt-in auto-start、hash-only 后台记录，以及安静持久化最后一行 Sensei 评分。Claude Code hooks 还会提供 compaction 后短上下文恢复。它们是可选的，并且只有相关 consent/settings 允许时才会生效。

Claude Code 会把 hooks 写入 `~/.claude/settings.json` 或 `.claude/settings.local.json`；Codex 会写入 `~/.codex/hooks.json` 或 `.codex/hooks.json`，并且可能需要用 `/hooks` 审核。

需要 hook setup 细节时，使用 `/prompt-sensei setup`、[docs/advanced-setup-zh.md](docs/advanced-setup-zh.md)、[examples/claude-settings.example.json](examples/claude-settings.example.json)，或 [examples/codex-hooks.example.json](examples/codex-hooks.example.json)。
